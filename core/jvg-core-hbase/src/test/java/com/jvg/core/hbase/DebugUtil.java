...
public final class DebugUtil {
	...
	public static String getContent(HTable t) throws IOException {
		...
		while (next != null && readCount < 1000) {
			...
			for (Map.Entry<byte[], NavigableMap<byte[], NavigableMap<Long, byte[]>>> cf : next
					.getMap().entrySet()) {
				for (Map.Entry<byte[], NavigableMap<Long, byte[]>> c : cf
						.getValue().entrySet()) {
					byte[] value = c.getValue().values().iterator().next();
					contentText.append(C_DEL);
					contentText.append(Bytes.toString(cf.getKey()))
							.append(CF_DEL).append(Bytes.toString(c.getKey()))
							.append(VAL_DEL).append(getText(value));
				}
			}
			contentText.append("\n");
			next = rs.next();
	...
	private static String getRowKeyAsText(byte[] row) {
		return Bytes.toString(new RowKeyDistributorByOneBytePrefix((byte) 0)
				.getOriginalKey(row))
				+ "-"
				+ Bytes.toLong(Bytes.head(
						Bytes.tail(row, 2 * Bytes.SIZEOF_LONG),
						Bytes.SIZEOF_LONG))
				+ "-"
