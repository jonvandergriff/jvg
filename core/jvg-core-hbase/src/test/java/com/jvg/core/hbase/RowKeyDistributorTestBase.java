...
public abstract class RowKeyDistributorTestBase {
	...
	public void testSimpleScanBounded() throws IOException {
		...
		byte[] stopKey = Bytes
				.toBytes(origKeyPrefix + seekIntervalMaxValue + 1);
		Scan scan = new Scan(startKey, stopKey);
		testSimpleScanInternal(origKeyPrefix, scan, 500, 500,
	...
	public void testSimpleScanWithoutStopKey() throws IOException {
		...
		int seekIntervalMinValue = 100;
		byte[] startKey = Bytes.toBytes(origKeyPrefix + seekIntervalMinValue);
		testSimpleScanInternal(origKeyPrefix, new Scan(startKey), 500, 500,
				100, 999);
	}

	/** Test scan with start and stop key. */
	@Test
	public void testMapReduceBounded() throws IOException,
		...
		byte[] stopKey = Bytes
				.toBytes(origKeyPrefix + seekIntervalMaxValue + 1);
		Scan scan = new Scan(startKey, stopKey);
		testMapReduceInternal(origKeyPrefix, scan, 500, 500,
				seekIntervalMinValue, seekIntervalMaxValue);
	}

	/** Test scan over the whole table. */
	@Test
	public void testMapReduceUnbounded() throws IOException,
			InterruptedException, ClassNotFoundException {
		long origKeyPrefix = System.currentTimeMillis();
	...
	private int writeTestData(long origKeyPrefix, int numRows, int rowKeySeed,
		...
		for (int i = 0; i < numRows; i++) {
			int val = rowKeySeed + i - i * (i % 2) * 2; // i.e. 500, 499, 502,
														// 497, 504, ...
			valuesCountInSeekInterval += (val >= seekIntervalMinValue && val <= seekIntervalMaxValue) ? 1
					: 0;
	...
	private void testSimpleScanInternal(long origKeyPrefix, Scan scan,
			int numValues, int startWithValue, int seekIntervalMinValue,
			int seekIntervalMaxValue) throws IOException {
		int valuesCountInSeekInterval = writeTestData(origKeyPrefix, numValues,
				startWithValue, seekIntervalMinValue, seekIntervalMaxValue);

		// TODO: add some filters to the scan for better testing
		ResultScanner distributedScanner = DistributedScanner.create(hTable,
				scan, keyDistributor);
		...
		for (Result current : distributedScanner) {
			countMatched++;
			if (previous != null) {
				byte[] currentRowOrigKey = keyDistributor
						.getOriginalKey(current.getRow());
				byte[] previousRowOrigKey = keyDistributor
						.getOriginalKey(previous.getRow());
				Assert.assertTrue(Bytes.compareTo(currentRowOrigKey,
						previousRowOrigKey) >= 0);
	...
	private void testMapReduceInternal(long origKeyPrefix, Scan scan,
			int numValues, int startWithValue, int seekIntervalMinValue,
			int seekIntervalMaxValue) throws IOException, InterruptedException,
	...
	/**
	 * Mapper that runs the count. NOTE: it was copied from RowCounter class
	 */
