 ...
 * with respect to row key distribution strategy
 * 
 * @author Alex Baranau
 */
public class WdTableInputFormat extends TableInputFormat {
	...
	public void setConf(Configuration conf) {
		...
		if (conf.get(ROW_KEY_DISTRIBUTOR_CLASS) != null) {
			...
			try {
				rowKeyDistributor = (AbstractRowKeyDistributor) Class.forName(
						clazz).newInstance();
				if (conf.get(ROW_KEY_DISTRIBUTOR_PARAMS) != null) {
					rowKeyDistributor
							.init(conf.get(ROW_KEY_DISTRIBUTOR_PARAMS));
				}
			} catch (Exception e) {
				throw new RuntimeException(
						"Cannot create row key distributor, "
								+ ROW_KEY_DISTRIBUTOR_CLASS + ": " + clazz, e);
			}
		}
	}
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException {
		...
		for (Scan scan : scans) {
			// Internally super.getSplits(...) uses scan object stored in
			// private variable,
			// to re-use the code of super class we switch scan object with
			// scans we
			setScan(scan);
