...
public class DistributedScanner implements ResultScanner {
	...
	public DistributedScanner(AbstractRowKeyDistributor keyDistributor,
			ResultScanner[] scanners) throws IOException {
	...
	public static DistributedScanner create(HTableInterface hTable,
			Scan originalScan, AbstractRowKeyDistributor keyDistributor)
			throws IOException {
	...
	private Result nextInternal(int nbRows) throws IOException {
		...
		for (int i = 0; i < nextOfScanners.length; i++) {
			...
			if (result == null
					|| Bytes.compareTo(keyDistributor
							.getOriginalKey(nextOfScanners[i].get(0).getRow()),
